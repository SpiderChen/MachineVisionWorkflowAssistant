"""自动登录流程（README §5，带开关；验证码留桩）。

登录的 UI 交互（输用户名 → 输密码 → 识别算术验证码 → 点登录）定义在内置流程
"login"（flows.yaml，流程编排页可视化编辑/重新框选）；验证码识别在 app/captcha.py
（1+1=? 图片算术题，OCR 解析），作为登录流程里的一个输入步骤执行。
